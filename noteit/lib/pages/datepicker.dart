...
class DatePickerState extends State<DatePicker> {
  DateTime dt; //
  TimeOfDay time; //
  final _formKey = GlobalKey<FormState>(); //text form
  ...
  FlutterLocalNotificationsPlugin fltrNotification =
      FlutterLocalNotificationsPlugin(); //initialize module

  List alarms = []; //list of alarms [[0,1,scheduledtime,counter,4]]
  int counter = 0; //
  List<Item> reminders; //list of reminder id
  String reminder;
  @override
  void initState() {
    //initialize
    super.initState();
    dt = DateTime.now(); //initialize date time
    time = TimeOfDay.now(); //initialize tme
  ...
  Future _sNotification(DateTime scheduledTime, int id) async {
    ...
    final location =
        await timeZone.getLocation(timeZoneName); // Find the 'current location'
